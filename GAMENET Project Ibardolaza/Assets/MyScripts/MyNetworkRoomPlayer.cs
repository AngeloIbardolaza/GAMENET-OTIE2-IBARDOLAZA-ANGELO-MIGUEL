...
public class MyNetworkRoomPlayer : NetworkRoomPlayer
{
    ...
    {
        //NetworkRoomManger.singleton.OnAddPlayer += RpcUpdatePlayerList;
    }
    ...
    {
        UpdatePlayerList(NetworkRoomManger.singleton.roomSlots);
    }
    ...
    public override void ReadyStateChanged(bool oldReadyState, bool newReadyState) 
    {
        ...
        if (isServer)
        {
            startButton.interactable = NetworkRoomManger.singleton.roomSlots.All(roomPlayer => roomPlayer.readyToBegin);
        }
    }
    ...
    public void StartGame()
    {
        ...
        NetworkRoomManger.singleton.ServerChangeScene(NetworkRoomManger.singleton.GameplayScene);
    }
    ...
    {
        List<MyNetworkRoomPlayer> roomPlayerList = playerList.GetComponentsInChildren<ReadyPlayerState>().Select(readyState => readyState.RoomPlayer).ToList();
        foreach(NetworkRoomPlayer player in players.Where(roomPlayer => !roomPlayerList.Contains(roomPlayer)))
        {
            ...
            obj.GetComponent<ReadyPlayerState>().SetRoomPlayer((MyNetworkRoomPlayer)player);
        }
        foreach(Transform t in playerList)
        {
            t.SetSiblingIndex(t.GetComponent<ReadyPlayerState>().RoomPlayer.index);
        }
    } 
